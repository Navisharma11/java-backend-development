...
//                // Require the user to be authenticated for all those requests.
        ...
        http.sessionManagement(session->session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))   //Here we are making our application stateless so that every request is consider as a new request doesn't matter whether there is a same user or different user
                // Adds the custom JWT filter to Spring Security's filter chain
                // and executes it before the UsernamePasswordAuthenticationFilter.
                // This allows the JWT to be validated and the user to be authenticated
...
//         daoProvider.setPasswordEncoder(NoOpPasswordEncoder.getInstance());
    ...
    @Bean // This AuthenticationManager object now i will hand over to spring IOC Container
    public AuthenticationManager authenticationManager(AuthenticationConfiguration config)   //AuthenticationManager checks whether the username/password (or other credentials) provided by the user are valid.
    {
        return config.getAuthenticationManager();   //With this method i am getting hold of object of authentication manager i am asking to AuthenticationConfiguration  "Give me the AuthenticationManager that you have configured."
    }
